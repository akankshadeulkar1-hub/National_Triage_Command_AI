...
export default function Navbar({ activeTab, onSelectTab, historyCount }: NavbarProps) {
  return (
    <header className="w-full bg-slate-900/90 backdrop-blur-md border-b border-slate-800 shadow-xl flex-shrink-0">
      <div className="max-w-[1800px] mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3 cursor-pointer" onClick={() => onSelectTab('dispatch')}>
          {/* Tactical Medical Radar Logo in Secure Emerald Theme */}
          <div className="relative flex items-center justify-center w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-600 via-emerald-800 to-slate-950 border border-emerald-500/60 shadow-[0_0_15px_rgba(16,185,129,0.4)] group overflow-hidden">
            <div className="absolute inset-0 bg-[radial-gradient(#10b981_1px,transparent_1px)] [background-size:6px_6px] opacity-30"></div>
            
            <svg
              className="w-6 h-6 text-white relative z-10 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] transition-transform group-hover:scale-110"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
          ...
          <div>
            <h1 className="text-lg font-black tracking-tight text-emerald-400 drop-shadow-[0_0_12px_rgba(16,185,129,0.5)] uppercase flex items-center space-x-2">
        ...
        <nav className="flex items-center space-x-2 bg-slate-950 p-1 rounded-xl border border-slate-800">
          <button
            onClick={() => onSelectTab('dispatch')}
            className={`flex items-center space-x-2 px-4 py-1.5 rounded-lg font-extrabold text-xs transition-all ${
              activeTab === 'dispatch'
          ...
          <Link
            href="/command-center"
            className="flex items-center space-x-2 px-4 py-1.5 rounded-lg font-extrabold text-xs text-red-400 hover:text-white hover:bg-red-950/60 border border-transparent hover:border-red-800/80 transition-all"
          >
          ...
          <button
            ...
            className={`flex items-center space-x-2 px-4 py-1.5 rounded-lg font-extrabold text-xs transition-all relative ${
              activeTab === 'history'
                ? 'bg-slate-800 text-white border border-slate-700 shadow-md'
                : 'text-slate-400 hover:text-white hover:bg-slate-900'
            }`}
          >
            ...
            {historyCount > 0 && (
              <span className="ml-1 bg-slate-800 text-slate-200 font-black text-[10px] px-2 py-0.5 rounded-full border border-slate-700">
                {historyCount}
              </span>
            )}
          </button>
